...
// @version      1.5.6
// @description  智能天赋选择(暴击优先/特殊词条排序)，自动挑战/重试/冥想处理，浏览器标题显示层数，天赋权重自配，主题日夜切换，呼吸灯提示，PC端标题栏收起/移动端悬浮球，面板位置记忆
// @author       耀
...
(function () {
    ...
    function updateThemeStyle() {
        ...
        .atp-panel { width: 320px; background: ${bg}; border: 2px solid ${border}; border-radius: 12px; box-shadow: ${isDark ? '0 0 30px rgba(0,0,0,0.6)' : '0 0 30px rgba(0,0,0,0.12)'}; overflow: hidden; display: flex; flex-direction: column; }
        .atp-header { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: ${isDark ? 'linear-gradient(90deg, rgba(201,153,58,0.12) 0%, rgba(201,153,58,0.06) 100%)' : 'linear-gradient(90deg, rgba(180,140,50,0.1) 0%, rgba(180,140,50,0.04) 100%)'}; border-bottom: 2px solid ${border2}; user-select: none; cursor: move; flex-shrink: 0; transition: all 0.3s, box-shadow 0.5s; }
        .atp-header.breathing { animation: atp-header-breathe 2s ease-in-out infinite; }
        @keyframes atp-header-breathe {
            0%,100% { box-shadow: 0 0 8px rgba(201,153,58,0.2); border-color: ${border}; }
            50% { box-shadow: 0 0 20px rgba(201,153,58,0.5), 0 0 40px rgba(201,153,58,0.2); border-color: ${isDark ? 'rgba(201,153,58,0.85)' : 'rgba(180,140,50,0.9)'}; }
        }
        ...
        .atp-body-wrap { overflow: hidden; transition: max-height 0.3s; flex: 1; min-height: 0; display: flex; flex-direction: column; }
        .atp-body { padding: 14px; display: flex; flex-direction: column; gap: 10px; max-height: 55vh; overflow-y: auto; -webkit-overflow-scrolling: touch; flex: 1; min-height: 0; }
        ...
        .atp-config-dialog { width: 380px; max-width: 100%; max-height: 85vh; background: ${bg}; border: 2px solid ${border}; border-radius: 12px; box-shadow: ${isDark ? '0 0 40px rgba(0,0,0,0.7)' : '0 0 40px rgba(0,0,0,0.2)'}; overflow: hidden; display: flex; flex-direction: column; }
        ...
        .atp-config-body { padding: 16px; display: flex; flex-direction: column; gap: 10px; overflow-y: auto; flex: 1; -webkit-overflow-scrolling: touch; }
        ...
        .atp-float-btn { position: fixed; width: 44px; height: 44px; border-radius: 50%; background: ${isDark ? 'linear-gradient(135deg, rgba(201,153,58,0.2) 0%, rgba(201,153,58,0.1) 100%)' : 'linear-gradient(135deg, rgba(180,140,50,0.18) 0%, rgba(180,140,50,0.08) 100%)'}; border: 2px solid ${border}; color: ${gold}; font-size: 20px; cursor: grab; z-index: 99998; display: flex; align-items: center; justify-content: center; user-select: none; -webkit-user-select: none; transition: box-shadow 0.5s; }
        ...
        @media (max-width: 768px) {
            ...
            .atp-body-wrap { flex: 1; min-height: 0; overflow-y: auto; -webkit-overflow-scrolling: touch; }
    ...
    function updateBreathingLight() {
        if (!headerEl || !floatBtn) return;
        const shouldBreathe = state.isRunning && (state.isCollapsed || !state.isPanelOpen);
        if (shouldBreathe) { headerEl.classList.add('breathing'); floatBtn.classList.add('breathing'); }
        else { headerEl.classList.remove('breathing'); floatBtn.classList.remove('breathing'); }
    }
    ...
    function renderBuffPanel() {
        ...
        els.bonusRow.innerHTML = [
            ['攻击', st.atkBonus || 0], ['血量', st.hpBonus || 0], ['灵力', st.mpBonus || 0],
            ['暴击', Math.round(st.critBonus || 0)], ['防御', st.defBonus || 0],
            ['吸血', Math.round(st.leechPercent || 0)], ['反伤', Math.round(st.thornPercent || 0)],
            ['连击', Math.round(st.doubleStrikeChance || 0)], ['技能触发', Math.round(st.skillTriggerBonus || 0)],
            ['层间回血', Math.round(st.healBonus || 0)]
        ].map(([l, v]) => `<span class="atp-bonus-item">${l}<span class="val${v > 0 ? ' positive' : ' zero'}">+${v}%</span></span>`).join('');
    ...
    function getBuffWeight(name, desc) {
        if (state.useDefaultStrategy) return getDefaultWeight(name, desc, isCritFull());
        const text = (name + ' ' + (desc || '')).toLowerCase();
        for (const kw of [...WEIGHT_KEYS_SPECIAL, ...WEIGHT_KEYS_NORMAL]) if (text.includes(kw)) return state.weights[kw] || DEFAULT_WEIGHTS[kw];
        return 5;
    }
    ...
    function formatBuffLine(buff, hl) {
        const isDark = currentTheme === 'dark' || (currentTheme === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches);
        const sym = buff.rarity === '传说' ? '⭐' : '◆';
        const name = buff.name || '?';
        const desc = buff.desc || '';
        const weight = getBuffWeight(name, desc);
        const prefix = hl ? '▶ ' : '';
    ...
    async function trialLoop() {
        ...
        if (state.autoRetry && state.isRunning && !state._stopRequested) {
            ...
            if (state.isRunning && !state._stopRequested) {
                addLog('info', '━━━ 自动重试 ━━━');
                state.currentFloor = 0; state._refreshAttempts = 0; updateTitle();
                if (state.autoMeditate) {
                    try { const sr = await apiGet('/api/game/meditate/status'); if (sr && sr.code === 200 && sr.data && sr.data.isMeditating) { addLog('info', '重试前先收功...'); await apiPost('/api/game/meditate/stop'); state._meditateActive = false; await wait(500); } } catch (e) {}
                }
                try {
                    const resetRes = await apiPost('/api/trial-tower/start', { useAdPoints: false });
                    if (resetRes && resetRes.code === 200) {
                        addLog('success', '试炼已重置');
                        if (state.autoMeditate) { try { const r = await apiPost('/api/game/meditate/start'); if (r && r.code === 200) { state._meditateActive = true; addLog('success', '冥想已重新开启'); } } catch (e) {} }
                        trialLoop(); return;
    ...
    function openConfigModal() {
        ...
        document.getElementById('atp-config-save').addEventListener('click', () => {
            const isDefault = btnDefault.classList.contains('active'); state.useDefaultStrategy = isDefault;
            state.autoRetry = document.getElementById('atp-cfg-auto-retry').checked;
            state.skipCombat = document.getElementById('atp-cfg-skip-combat').checked;
            state.stoneRefresh = document.getElementById('atp-cfg-stone-refresh').checked;
            state.refreshMaxAttempts = parseInt(document.getElementById('atp-cfg-refresh-max').value, 10) || 3;
            state.autoMeditate = document.getElementById('atp-cfg-auto-meditate').checked;
            state.stopOnFloor = parseInt(document.getElementById('atp-cfg-stop-floor').value, 10) || 0;
            state.showFloorInTitle = document.getElementById('atp-cfg-show-floor').checked;
            state.titleTemplate = document.getElementById('atp-cfg-title-template').value.trim() || '天道塔挑战中【{floor}】层';
            if (!isDefault) [...WEIGHT_KEYS_SPECIAL, ...WEIGHT_KEYS_NORMAL].forEach(k => { const inp = overlay.querySelector(`#atp-weight-${k}`); if (inp) state.weights[k] = parseInt(inp.value, 10) || DEFAULT_WEIGHTS[k]; });
            saveConfig(); updateTitle(); addLog('info', `配置已保存 (${isDefault ? '默认策略' : '自配权重'})`); closeConfigModal();
        });
    ...
    function makeDraggable(handle, target, onEnd, ew, eh) {
        ...
        function mv(e) { if (!d) return; const w = ew || target.offsetWidth, h = eh || target.offsetHeight; target.style.left = clamp(ol + e.clientX - sx, 0, innerWidth - w) + 'px'; target.style.top = clamp(ot + e.clientY - sy, 0, innerHeight - h) + 'px'; }
        function up() { if (!d) return; d = false; target.style.transition = ''; const r = target.getBoundingClientRect(); if (onEnd) onEnd({ left: clamp(r.left, 0, innerWidth - (ew || target.offsetWidth)), top: clamp(r.top, 0, innerHeight - (eh || target.offsetHeight)) }); }
    ...
    function buildPanel() {
        ...
        makeDraggable(headerEl, container, pos => saveJSON(POS_KEY, pos), 320, state.isCollapsed ? 40 : 400);
        updateTitle(); updateBreathingLight();
    }

    function bindEvents() {
        document.getElementById('atp-theme-btn').addEventListener('click', cycleTheme);
    ...
    function init() { if (document.getElementById('atp-container')) return; loadConfig(); initThemeWatcher(); updateThemeStyle(); buildPanel(); bindEvents(); addLog('info', '盐值验证通过'); addLog('info', `自动试炼塔 v1.5.6 已加载 (${state.useDefaultStrategy ? '默认策略' : '自配权重'})`); updateAllStats(); }
